...
class PTL():
    ...
    def update_state_visits(self, p, state, action):
        q = torch.cat((state, action[0]), 0) 
        return -1 * self._curiosity[p].update(q)

    def confidence(self, p, state, action):
        q = torch.cat((state, action[0]), 0) 
        return -1 * self._curiosity[p].uncertainty(q).item()
